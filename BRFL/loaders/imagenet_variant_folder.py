...
def define_imagenet_variant_folder(root, data_name, transform, train=False, img_size=None):
...
class ImageNetValNaturalShift(object):
    ...
    def __init__(self, root, data_name, transform=None, version=None):
        ...
        # for imagenet-v2, modify folder name before loading, otherwise the order is wrong.
        if "imagenet_v2" in self.data_name:
            file_names = sorted(os.listdir(self.path_data_and_labels))
            file_names_padded = ["{:0>4}".format(f) for f in file_names]
            for i in range(len(file_names_padded)):
                os.rename(os.path.join(self.path_data_and_labels, file_names[i]), os.path.join(self.path_data_and_labels, file_names_padded[i]))
