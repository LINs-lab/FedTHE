...
def get_corr_data(data, indices_per_client, random_state, severity=5):
    ...
    for i, ind in indices_per_client.items():
        ...
        corr_test[i] = CorruptData(data=curr_data, random_state=random_state, severity=severity)
    return corr_test
...
def get_natural_shift_data(data, indices_per_client, random_state, data_path, data_name):
    ...
    for i in range(len(partitioned_indices)):
        natural_shift_test[i] = torch.utils.data.Subset(
            natural_shift_data, partitioned_indices[i]
        )

    return natural_shift_test

def get_mixed_data(fl_data_per_client, random_state):
    mixed_test = {}
    for i in range(len(fl_data_per_client["test"])):
        ...
        id_ood_tests = torch.utils.data.ConcatDataset(id_ood_tests)
        ...
        mixed_test[i] = torch.utils.data.Subset(id_ood_tests, ind)
    return mixed_test
...
class CorruptData(torch.utils.data.Dataset):

    def __init__(self, data, random_state, severity):
        # initialize corruption class
        self.corruptions = Corruptions(random_state, severity)
...
class Corruptions(object):
    ...
    def __init__(self, random_state, severity=5):
