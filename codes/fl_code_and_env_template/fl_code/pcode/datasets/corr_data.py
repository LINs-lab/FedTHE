...
"""
Some corruptions are referred to 
    https://github.com/hendrycks/robustness/blob/master/ImageNet-C/create_c/make_cifar_c.py
"""


def define_corr_data(data, seed, severity=5):
    return CorruptData(data=data, seed=seed, severity=severity)
...
class CorruptData(object):
    ...
    def __init__(self, data, seed, severity):
        # initialize corruption class
        self.corruptions = Corruptions(seed, severity)
        self.data_size = len(data)
        if type(data) is partition_data.Partition:
            self.data = data.data
    ...
    def __getitem__(self, index):
        ...
        corr_data, target = self.corruptions.apply(self.data[data_idx])
        return corr_data, target
...
class Corruptions(object):
    ...
    def apply(self, data):
        _img, label = data[0], data[1]  # [3, 32, 32]
        # unnormalize = trn.Compose([trn.Normalize(mean=[0., 0., 0.], std=[1/0.2023, 1/0.1994, 1/0.2010]),
        #                         trn.Normalize(mean=[-0.4914, -0.4822, -0.4465], std=[1., 1., 1.])])
        unnormalize = trn.Compose(
            [
                trn.Normalize(mean=[0.0, 0.0, 0.0], std=[1 / 0.5, 1 / 0.5, 1 / 0.5]),
                trn.Normalize(mean=[-0.5, -0.5, -0.5], std=[1.0, 1.0, 1.0]),
            ]
        )
        convert_img = trn.Compose([unnormalize, trn.ToPILImage()])
        pil_img = convert_img(_img)
        corruption = self.random_state.choice(self.corruption_list)
        corr_img = np.uint8(corruption(pil_img, self.random_state, self.severity))
        post = trn.Compose(
            [trn.ToTensor(), trn.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
        )
        corr_img = post(corr_img)

        return corr_img, label
...
def frost(x, random_state, severity=1):
    ...
    filename = [
        "./pcode/datasets/cifar10c_resource/frost1.png",
        "./pcode/datasets/cifar10c_resource/frost2.png",
        "./pcode/datasets/cifar10c_resource/frost3.png",
        "./pcode/datasets/cifar10c_resource/frost4.jpg",
        "./pcode/datasets/cifar10c_resource/frost5.jpg",
        "./pcode/datasets/cifar10c_resource/frost6.jpg",
    ][idx]
