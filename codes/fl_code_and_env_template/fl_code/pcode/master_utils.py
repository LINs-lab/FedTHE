...
def do_validation(
    ...
    if label == "global_model":
        ...
        conf.logger.log(f"Master finished the validation for comm. round={comm_round}.")
        if not conf.train_fast:
            ...
            conf.logger.log(f"Master updated checkpoint at comm. round={comm_round}.")
...
def get_avg_perf_on_dataloaders(
    ...
    print(f"\tGet averaged global performance from {len(data_loaders)} data_loaders.")
    performance = []
