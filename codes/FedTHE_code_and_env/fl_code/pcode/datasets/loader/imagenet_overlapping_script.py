...
imagenet_r_indices = [i for i in range(len(all_wnids)) if imagenet_r_mask[i] is True]
imagenet_a_indices = [i for i in range(len(all_wnids)) if imagenet_a_mask[i] is True]

if __name__ == "__main__":
    print(len(all_wnids))
    print(len(imagenet_a_wnids))
    # print(imagenet_r_indices)
    # print(imagenet_a_indices)
    overlapping_cls = np.intersect1d(imagenet_a_indices, imagenet_r_indices).tolist()
    # print(overlapping_cls)

    a_dict = {}
    for i, a in enumerate(imagenet_a_indices):
        a_dict[a] = i
    print(a_dict)
    a_cls = [a_dict[o] for o in overlapping_cls]
    print(a_cls)
    print(len(a_cls))

    r_dict = {}
    for i, r in enumerate(imagenet_r_indices):
        r_dict[r] = i
    print(r_dict)
    r_cls = [r_dict[o] for o in overlapping_cls]
    print(r_cls)
    print(len(r_cls))
    ...
    # overlapping_cls = [6, 11, 13, 22, 23, 39, 47, 71, 76, 79, 90, 94, 96, 97, 99, 105, 107, 113, 125, 130, 132, 144, 150, 151, 207, 234, 235, 254, 277, 291, 301, 308, 309, 310, 311, 314, 315, 319, 323, 327, 330, 334, 335, 347, 361, 372, 397, 401, 407, 425, 428, 430, 437, 457, 462, 470, 472, 483, 579, 609, 658, 701, 763, 768, 774, 776, 779, 780, 815, 820, 833, 847, 907, 932, 933, 934, 937, 943, 945, 947, 951, 954, 957, 980, 981, 988]

    # NOTE: uncomment following code only for the first time of run, because their filename cause trouble.
    # import os, glob
    # print(os. getcwd())
    # for path in glob.glob('imagenetv2-matched-frequency-format-val'):
    ...
    for l in lines:
        temp = l.split(" ")
        imagenet_dict[temp[0]] = int(temp[1])

    selected_cls = [all_wnids[i] for i in v2_cls]
    imagenet_cls = [imagenet_dict[cls] for cls in selected_cls]
    print(imagenet_cls)
    print(len(imagenet_cls))
